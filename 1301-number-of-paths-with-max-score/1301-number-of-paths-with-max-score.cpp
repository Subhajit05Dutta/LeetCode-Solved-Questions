class Solution {
    ...
    vector<vector<pair<int, int>>>
        memo; // cache; first == INT_MIN means "not solved yet"
    ...
    pair<int, int> solve(int i, int j) {
        ...
        if (memo[i][j].first != INT_MIN)
            return memo[i][j]; // <-- already computed, reuse
...
class Solution {
    ...
    pair<int, int> solve(int i, int j) {
        ...
        if (board[i][j] == 'X')
            return {-1, 0};
        int val = (board[i][j] == 'S') ? 0 : board[i][j] - '0';
        pair<int, int> up = solve(i - 1, j);       // up
        pair<int, int> left = solve(i, j - 1);     // left
        pair<int, int> diag = solve(i - 1, j - 1); // up-left
        int best = max({up.first, left.first, diag.first});
        if (best == -1)
            return {-1, 0};
        int way = 0;
        if (up.first == best)
            way = (way + up.second) % MOD;
        if (left.first == best)
            way = (way + left.second) % MOD;
        if (diag.first == best)
            way = (way + diag.second) % MOD;
        return {best + val, way};
    }
