...
public:
    vector<int> maxActiveSectionsAfterTrade(string s,
        ...
        auto clip = [&](int j, int l,
                        int r) { // valley j's gain, clipped to [l, r]
        ...
        auto gain = [&](int l, int r) {
            if (nblocks < 2)
                return 0;
            int ja = ranges::lower_bound(ze, l) -
                     ze.begin(); // first usable valley: left block ends >= l
            int jb = ranges::upper_bound(zs, r) - zs.begin() -
                     2; // last  usable valley: right block starts <= r
            if (ja > jb)
                return 0;
            return max({clip(ja, l, r), clip(jb, l, r),
